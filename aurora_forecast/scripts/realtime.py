#!/usr/bin/env python3
# =============================================================================
# End-to-end realtime Aurora forecast pipeline.
# =============================================================================
# One command drives the whole production loop:
#
#     download latest ECMWF open-data analysis
#       -> adapt GRIB into the Aurora input .nc
#       -> submit GPU inference to Slurm (and wait)
#       -> render the visualization figure set
#
# Stages 1-2 (download + adapt) are light CPU work on the login node. Stage 3
# (inference) is the only GPU step, run via scripts/run_inference.sbatch.
# Stage 4 (visualization) reuses the shared weathernext_forecast visualizer
# against the unified results schema.
#
...
# (opendata_download, weathernext_forecast) live one level up, under a shared
# repo root (locally `forecast_models/`, on Pawsey `Foehn/`).
# -----------------------------------------------------------------------------
PROJECT_DIR = Path(__file__).resolve().parents[1]   # aurora_forecast
REPO_ROOT = PROJECT_DIR.parent                        # shared repo root

for _pkg in ("opendata_download",):
...
from aurora_forecast import adapter                      # noqa: E402
...
VIZ_SCRIPT = REPO_ROOT / "weathernext_forecast" / "scripts" / "visualize.py"
...
def stage_adapt(date, hour, raw_root, out_dir, force) -> str:
    print("[2/4] adapting GRIB -> Aurora input .nc", flush=True)
    path = adapter.build_input(date, hour, raw_root=raw_root, out_dir=out_dir, force=force)
...
def stage_submit(input_name: str, date, hour, args) -> None:
    ...
    cmd = [
        ...
        "--time", args.walltime,
        "--job-name", f"au_{date:%Y%m%d}_{hour:02d}",
        "--output", str(log_dir / f"infer_{date:%Y%m%d}_{hour:02d}.%j.out"),
        "--chdir", str(PROJECT_DIR),
        "--export", f"ALL,AURORA_INPUT_FILENAME={input_name},CONDA_ENV={args.conda_env}",
    ]
    if not args.no_wait:
        cmd.append("--wait")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="End-to-end realtime Aurora forecast.")
    ...
    g.add_argument("--gpus-per-node", type=int, default=1,
                   help="GPUs requested (maps to --gres=gpu:N; 1 GCD is ample for Aurora)")
    g.add_argument("--walltime", default="02:00:00", help="Slurm walltime HH:MM:SS")
    g.add_argument("--conda-env",
                   default="/scratch/pawsey0115/hwang4/miniconda3/envs/aurora-gpu")
    g.add_argument("--no-submit", action="store_true", help="download + adapt only")
    ...
    g.add_argument("--no-visualize", action="store_true",
                   help="skip rendering (reuses the shared weathernext visualizer)")
    ...
    # Aurora consumes the static fields from its pickle, not from GRIB; the shared
    # downloader's static step is still harmless to run but unnecessary here.
    stage_download(client, date, hour, args.data_root, args.force, include_static=False)
    input_name = stage_adapt(date, hour, raw_root, out_dir, args.force)
